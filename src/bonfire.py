...
class bonfire:
	...
	def generateMessage(self, message):
		...
		name = f"[b][color=block]{self.config.DEVPREFIX}{'[IRC] ' if str(message.webhook_id) == str(self.config.TIPLANET.irc.id) else ''}{self.removeDiscordID(message.author)}[/color][/b]: "
